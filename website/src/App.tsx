  ...
  users: [
    {
      ...
      data: {
        ...
        preferences: {
          theme: 'dark',
          sceneType: 'ip', // Triggers event IP layout logic in real app
...
const renderPrimitiveValue = (val: any) => {
  ...
  if (typeof val === 'number') {
    return <span className="text-text-primitive-num text-[#b5cea8] font-mono">{val}</span>;
  }
  if (typeof val === 'string') {
    return <span className="text-text-primitive-str text-[#ce9178] font-mono">"{val}"</span>;
  }
...
export default function App() {
  ...
  // Auto trigger gateway prefill connection
  ...
  const handleConnect = async (e: React.FormEvent) => {
    ...
    if (gatewayMode === 'local') {
      ...
      addToast('Connecting', 'Initializing Firebase SDK...', 'info');
      try {
        const configObj = JSON.parse(liveConfig);
        if (!configObj.projectId) throw new Error("Invalid config: Missing projectId");
        
        // Initialize or get existing app to avoid duplicate init errors
        if (getApps().length === 0) initializeApp(configObj);
        // Attempt a test fetch to verify permissions (simulated for now)
        addToast('Verifying', 'Checking Firestore security rules...', 'info');
        
        // Just setting the session for now. Real fetches would happen in the explorer.
        setActiveSession({ mode: 'live', host: 'firestore.googleapis.com', project: configObj.projectId });
        addToast('Connected Successfully', `Live connected to project '${configObj.projectId}'.`, 'success');
        setSimScreen('explorer');
      } catch (err: any) {
        if (err.code === 'permission-denied') {
           addToast('Permission Denied', 'Your Firestore rules require authentication. Please temporarily allow public reads.', 'error');
        } else {
           addToast('Connection Failed', err.message || 'Invalid config format.', 'error');
        }
      } finally {
        setSimConnecting(false);
      }
    ...
    <>
      ...
      <div className="container">
        ...
        <section className="hero-section">
          <h1>Universal Firestore Database Explorer</h1>
          <p className="description">
            Zero-friction database visualizer for Local Emulators & Live Production. Inspect collections, browse nested fields, analyze structures with syntax highlighting, and download native single-click exports.
          </p>
        ...
        <section className="screenshot-showcase">
          <div className="simulator-window">
            ...
            <div className="sim-titlebar">
              ...
              <div>
                <span className="sim-badge connected">
                  <span style={{ color: '#27c93f', marginRight: '4px' }}>●</span> Connected
                </span>
              </div>
            </div>
            ...
            {simScreen === 'gateway' && (
              <div className="sim-gateway-screen" style={{ padding: '2rem' }}>
                <div className="sim-gateway-card" style={{ 
                  width: '100%', maxWidth: '600px', margin: '2rem auto',
                  background: '#111115', border: '1px solid #1e1e24', borderRadius: '12px', padding: '2rem',
                  boxShadow: '0 12px 32px rgba(0,0,0,0.1)'
                }}>
                  <div className="sim-gateway-logo" style={{ marginBottom: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <Database size={48} color="#FFCA28" style={{ marginBottom: '1rem' }} />
                    <span style={{ fontSize: '1.4rem', fontWeight: 600 }}>Connect to Database</span>
                  </div>

                  <div className="sim-mobile-tabs" style={{ display: 'flex', marginBottom: '2rem', background: '#0a0a0c', padding: '0.4rem', borderRadius: '8px', borderBottom: 'none' }}>
                    <button 
                      type="button"
                    ...
                    {gatewayMode === 'local' ? (
                      <>
                        <div className="form-group" style={{ marginBottom: '1.5rem' }}>
                          <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.85rem', color: '#8c889a' }}>Emulator Host</label>
                          <input 
                            type="text" 
                            className="sim-input" 
                            value={localHost} 
                            ...
                            required
                            style={{ width: '100%', padding: '0.8rem', background: '#0a0a0c', border: '1px solid #1e1e24', borderRadius: '6px', color: '#fff' }}
                          />
                        </div>
                        <div className="form-group" style={{ marginBottom: '2rem' }}>
                          <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.85rem', color: '#8c889a' }}>Project ID</label>
                          <input 
                            type="text" 
                            className="sim-input" 
                            value={localProject} 
                      ...
                      <>
                        <div className="form-group" style={{ marginBottom: '1rem' }}>
                          <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.85rem', color: '#8c889a' }}>Firebase Config JSON</label>
                          <textarea 
                            className="sim-input" 
                            value={liveConfig} 
                            onChange={(e) => setLiveConfig(e.target.value)} 
                            placeholder='{"apiKey": "...", "authDomain": "...", "projectId": "..."}' 
                            style={{ width: '100%', minHeight: '120px', padding: '0.8rem', fontFamily: 'monospace', background: '#0a0a0c', border: '1px solid #1e1e24', borderRadius: '6px', color: '#fff' }}
                            required
                          />
                        </div>
                        <div style={{
                          ...
                          <Info size={16} color="#27c93f" style={{ flexShrink: 0, marginTop: '2px' }} />
                          <div>
                            <strong style={{ color: '#27c93f' }}>Privacy First:</strong> We do not store your Firebase credentials. All connections are made directly from your browser to Google's servers. 
                          </div>
                        </div>
                      </>
                    )}

                    <div className="sim-gateway-actions" style={{ display: 'flex', justifyContent: 'flex-end' }}>
                      <button 
                        type="submit" 
                        className="sim-btn-primary"
                        style={{ padding: '0.8rem 2rem', border: 'none', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', fontWeight: 600 }}
            ...
            {simScreen === 'explorer' && (
              <div className="sim-explorer-screen">
                ...
                <div className={`sim-pane sim-col-collections ${activeMobileTab === 'collections' ? 'mobile-visible' : 'mobile-hidden'}`}>
                  <div className="sim-pane-header">
                    ...
                    <button className="sim-icon-btn" onClick={handleSimRefresh} title="Sync indexes">
                      <RefreshCw size={14} className={simRefreshing ? 'animate-spin' : ''} />
                    </button>
                  </div>
                  ...
                  <div className="sim-pane-content">
                    {filteredDocs.length === 0 ? (
                      ...
                      filteredDocs.map((doc) => (
                        ...
                        >
                          <span style={{ fontSize: '10px', color: '#ff9100' }}>📄</span>
                ...
                {/* Panel 3: Fields Tree view (Dark Code style) */}
                <div className={`sim-pane sim-col-fields ${activeMobileTab === 'fields' ? 'mobile-visible' : 'mobile-hidden'}`}>
                  ...
                  <div className="sim-pane-content sim-fields-workspace">
                    {activeDoc ? (
                      <div>
                        <div style={{ color: '#6a9955', marginBottom: '0.8rem', fontSize: '0.78rem', userSelect: 'none' }}>
                          // Root Path: {selectedCol}/{selectedDocId}
                        </div>
                      ...
                      <div className="sim-empty-state" style={{ color: '#a49fb2' }}>
                        <Database size={28} />
                  ...
                  <div style={{
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <span style={{ color: '#ffca28', fontSize: '0.9rem' }}>●</span> 
                      Session: {activeSession.host} ({activeSession.project})
                    </span>
                    <span style={{ color: '#ffca28', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <Database size={12} /> Live Visualizer
                    </span>
                  </div>
                </div>
